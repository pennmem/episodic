...
class Events(object):
    ...
    def log_gold_view_events(self):
        ...
        _trials = self.keep_trials if self.remove_bad_trials else self.trials
        #cjm change start
        _list_of_items = ["rubyheartInView", "amethysttorusInView", 
                          "saphireteardropInView", "citrinegeoInView", 
                          "diamondroundInView", "yellowmoonInView", 
                          "emeraldrectangleInView","golddepositInView"]
        if (1): 
            gold_events = self.events.query("(trial=={}) & (key in @_list_of_items)".format(_trials)).copy()
        else:
    ...
    def log_gold_spawned(self):
        ...
        #cjm change start
        _list_of_items = ["citrineGeoLocation", "saphireTeardropLocation", 
                          "rubyHeartLocation", "amethystTorusLocation", 
                          "goldDepositLocation", "emeraldRectangleLocation", 
                          "yellowMoonLocation","diamondRoundLocation"]        
        if (1): 
            gold_spawned = self.events.query("(trial=={}) & (key in @_list_of_items)".format(_trials)).copy()
        else:
        ...
        if (1):
            dig_events['gold_id']= dig_events.apply(
                lambda x: self._nearest_gold_id(x['trial'],
                                                    x['value']['nearestItemPositionX'],
                                                    x['value']['nearestItemPositionZ']),
                                                    axis=1)
            dig_events['dig_success']=dig_events['value'].apply(lambda x: x['successful'])
            dig_events['dist_from_gold']=dig_events['value'].apply(lambda x: x['distanceFromNearestItem'])
