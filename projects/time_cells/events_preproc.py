...
def read_events_json(subj_sess,
    ...
    tmp_txt = events['time'][0].item()
    print(f'The first timestamp in events before zero-ing is {tmp_txt:.3f}')

    if start_at_zero:
        events['time'] = (events['time'] - events.at[0, 'time'])
        tmp_txt = events['time'][0].item()
        print(f'The first timestamp in events after zero-ing is {tmp_txt:.3f}')
...
def load_syncs(subj_sess,
    ...
    try:
        matfile_contents = sio.whosmat(sync_f) ;
        if not [element for element in matfile_contents if 'TimeStamps' in element]:  
            sync_chan = np.squeeze(sio.loadmat(sync_f)[data_key])
        else:
            sync_chan = np.squeeze(sio.loadmat(sync_f)['TimeStamps'])
            TTL = np.squeeze(sio.loadmat(sync_f)['TTLs'])
            timestamp_flag = 1
    except NotImplementedError:
        with h5py.File(sync_f, 'r') as f:           
            if not [element for element in f.keys() if 'TimeStamps' in element]:
                sync_chan = np.squeeze(f[data_key])
            else:
                timestamp_flag = 1
                [element for element in f.keys() if 'TimeStamps' in element]
    return sync_chan, TTL, timestamp_flag
...
def format_events(subj_sess=None,
    ...
    for trial in range(1, events['trial'].max() + 1):
        for game_state in ['Delay', 'ReturnToBase']:
            ...
            print(f"Processing trial {trial} and game state {game_state}")
