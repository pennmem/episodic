...
def load_lfp_timestamps(subj_sess=None,
    ...
    if op.exists(lfp_timestamps_f):
        ...
        print( f'The first timestamp in lfp_timestamps before zero-ing & after conv factor is {lfp_timestamps[0]:.3f}' )
            
        if start_at_zero:
            lfp_timestamps -= lfp_timestamps[0]
            
        print( f'The first timestamp in lfp_timestamps after zero-ing & conv factor is {lfp_timestamps[0]:.3f}' )
