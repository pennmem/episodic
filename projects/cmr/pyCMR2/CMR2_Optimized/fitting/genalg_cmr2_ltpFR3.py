...
def ga(func, lb, ub, data_pres, identifiers, w2v, sources, target_stats, 
    ...
    for i, gen in enumerate(range(1, ngen + 1)):
        ...
        sources_gen = sources[session_mask] if sources is not None else None
...
def run_ga(data_pres, sessions, w2v, sources, targets):
    ...
    # GA settings: Settings are defined as lists, where the nth list item defines the settings for the nth "epoch" of generations. An epoch is defined as set of generations with identical settings, and there can be any number of generations in a given epoch.
    ngen = [5, 10, 15, 10, 5]  # Number of generations to run in each epoch
    popsize = [2000, 1600, 1200, 800, 400]  # Population size
    parent_rate = [.2, .2, .2, .2, .2]  # The fraction of individuals that will be allowed to become parents for the next generation
    cross_rate = [.5, .5, .5, .5, .5]  # The probability of each parameter being passed from parent 1 (as opposed to parent 2)
    mut_rate = [1, 1, 1, 1, 1] # The probability of each parameter mutating in a child
    mut_scale = [.2, .1, .05, .025, .0125] # The standard deviation of mutations, relative to the full range of that parameter (.1 = 10% of range)
    nsess = [350, 500, 750, 1000, 1469]  # The number of sessions to simulate with the model
    ...
    first_gen = None #np.loadtxt('/scratch/jpazdera/cmr2/outfiles/first_gen.txt')
    
    #    [b_e, b_r, g_fc, g_cf, p_s, p_d,   k,   e, s_cf, b_rp,   o,    a, c_t,  l]
    lb = [.2,   .2, .500, .500,   0,   0,   0, .01,    0, .500,  1., .500,   0,  0]
    ub = [.8,   .8, .999, .999,   4,   3,  .4, .75,    5, .999, 20., .999,  .5, .3]
    ...
    np.savetxt(OUTDIR + 'xopt_ltpFR3.txt', xopt, delimiter=',', fmt='%f')
...
if __name__ == "__main__":
    ...
    # Interleave visual and auditory sessions to make sure we simulate equal numbers of each
    file_list = [f for pair in zip(vfile_list, afile_list) for f in pair]
