...
class CMR2(object):
    ...
    def create_semantic_structure(self):
        ...
        all_patterns = self.all_session_items
    ...
    def leaky_accumulator(self, double [:] in_act, double [:, :] noise_vec,
                                                double [:] x_thresholds):
        """
        ...
        """
        cdef double lamb, kappa, dt_tau, sq_dt_tau
        ...
        cdef int ncycles, nitems_in_race
        ...
        cdef double [:] winners
        winners = np.zeros(nitems_in_race)

        ## declare winner_vec
        cdef double [:] winner_vec
    ...
    def recall_session(self):
        ...
        while ((time_passed < rec_time_limit)
            ...
            # test without any noise
            # noise = np.zeros((nitems_in_race, int(max_cycles)))
...
def run_CMR2(LSA_path, LSA_mat, data_path, params, sep_files, filename_stem=""):
    ...
    # massive data matrix.
        ...
        for file_path in indiv_file_paths:
            ...
            try: data_mat = data_file['data'].pres_itemnos  # get presented items
            except AttributeError:
                print 'Skipped one file. This is most likely due to one subject' + \
                ' having no data and is not a cause for concern'
                continue
        ...
        data_file = scipy.io.loadmat(indiv_file_paths[1], squeeze_me=True,
