...
def separate_files(data_path):
    ...
    subj_id_map = np.repeat(np.arange(1,(
                data_pres_list_nos.shape[0]/(data_pres_list_nos.shape[1]+1))),
                    data_pres_list_nos.shape[1])
    unique_subj_ids = np.unique(subj_id_map)

    # np.savetxt('unique_subj_ids_testing.txt', subj_id_map, fmt='%i')

    # raise ValueError("stop EVERYTHING")
...
def run_CMR2(LSA_path, LSA_mat, data_path, params, sep_files, filename_stem=""):
    ...
    if not sep_files:
        ...
        print(subj_presented_data[0])
        # raise ValueError("stop and check presented data")
    ...
    print("I am zero-padding the output")
...
def main():
    ...
    if on_rhino:
        LSA_path = 'w2v.txt'
        data_path = 'pres_nos_LTP228_x4.txt'
    else:
    ...
    # test issue with .txt file
    #LSA_mat = scipy.io.loadmat(
    #    'PEERS_LSA.mat', squeeze_me=True, struct_as_record=False)['LSA']
    ...
    # save CMR2 results
    np.savetxt('resp_LTP228_x4.txt', np.asmatrix(rec_nos), delimiter=',', fmt='%i')
    np.savetxt('times_LTP228_x4.txt', np.asmatrix(times), delimiter=',', fmt='%i')
    print 'shai edited succesfully'
