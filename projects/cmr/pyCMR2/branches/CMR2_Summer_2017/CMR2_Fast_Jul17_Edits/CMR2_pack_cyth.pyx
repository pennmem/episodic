...
class CMR2(object):
    ...
    def __init__(self, params, nsources, source_info, LSA_mat, data_mat):
        ...
        self.M_FC = np.identity(self.nelements, dtype=np.float32) * self.params['scale_fc']
        self.M_CF = np.identity(self.nelements, dtype=np.float32) * self.params['scale_cf']
...
def separate_files_by_session(data_path):
    """If data is in one big file, separate out the data into sheets, by subject session pair.

    :param data_path: Include a path to the data. It should be a .mat file
    :return: a list of data matrices, separated out by individual subject-sessions. Also returns
    ids for subject-session stuff.
    ...
    # Basically presented items for each unique session-subject pair
    split_data = []
    ...
    subject_vector = data_file['data'].subject
    session_vector = data_file['data'].session
    #subject session mix - basically just isolating unique subject + session
    ss_mix = np.array([str(subject_vector[i]) + '_' +  str(session_vector[i]) for i in xrange(len(subject_vector))])
    #grab unique values and use as a boolean mask
    unique_ids = np.unique(ss_mix)
    for unique_ss in unique_ids:
        split_data.append(data_pres_list_nos[ss_mix == unique_ss])
    
    #Old code that splits ll in half for runtime insight
    #print 'WARNING: In debug mode: splitting list length in half - line 945'
    #halved_split = [np.array([item[0:12] for item in session]) for session in split_data]
    #return halved_split, ss_mix
...
def run_CMR2(LSA_path, LSA_mat, data_path, params, sep_files, filename_stem="", separate_by_session = False):
    ...
    if not sep_files:
        ...
        if separate_by_session == True:
            print('Separating by session')
            #names are misleading since this feature was added after separate_files
            #and we are going by session not subject
            #May be a good idea to change the names eventually
            subj_presented_data, unique_subj_ids = separate_files_by_session(data_path)
        else:
            print('Separating by subject')
...
def wrapper(out, LSA_path, LSA_mat, data_path, params_K02, sep_files, separate_by_session):
    answera, answerb = run_CMR2(LSA_path=LSA_path, LSA_mat=LSA_mat,
                              data_path=data_path,
                              params=params_K02, sep_files = sep_files,\
                                separate_by_session = separate_by_session)
    out.append(answera)
    out.append(answerb)
    return
def cProf(LSA_mat, data_path,ll, separate_by_session, sep_files, doProfile = True):
    # set desired parameters. Example below is for Kahana et al. (2002),
    # from Lohnas et al. (2015)
    LSA_path = 'not important'
    params_K02 = {
    ...
    if doProfile == True:
        outs = []
        cProfile.runctx("wrapper(outs, LSA_path, LSA_mat, data_path, params_K02, sep_files, separate_by_session)", globals(), locals(), "Profile.prof")
        ...
        rec_nos, times = run_CMR2(LSA_path=LSA_path, LSA_mat=LSA_mat,
                              data_path=data_path,
                              params=params_K02, sep_files=sep_files, separate_by_session = separate_by_session)
    print("End of time: " + str(time.time() - start_time))
    np.savetxt('resp_profiler.txt', np.asmatrix(rec_nos), delimiter=',', fmt='%.0d')
    np.savetxt('times_profiler.txt', np.asmatrix(times), delimiter=',', fmt='%.0d')
...
def main():
    ...
    on_rhino = True
    if on_rhino:
        LSA_path = '/home1/rivkat.cohen/PycharmProjects/testing_Jul8/CMR2_lowmem/K02_LSA.txt'
        data_path = '/home1/rivkat.cohen/PycharmProjects/K02_files/K02_data.mat'
    else:
        #LSA_path = '/Users/KahaNinja/PycharmProjects/CMR2/K02_files/K02_LSA.mat'
        LSA_path = 'K02_LSA.txt'
    ...
    rec_nos, times = run_CMR2(LSA_path=LSA_path, LSA_mat=LSA_mat,
                              data_path=data_path,
                              params=params_K02, sep_files=False)
    print("End of time: " + str(time.time() - start_time))

    # save CMR2 results
    np.savetxt('resp_K02.txt', np.asmatrix(rec_nos), delimiter=',', fmt='%i')
    np.savetxt('times_K02.txt', np.asmatrix(times), delimiter=',', fmt='%i')
