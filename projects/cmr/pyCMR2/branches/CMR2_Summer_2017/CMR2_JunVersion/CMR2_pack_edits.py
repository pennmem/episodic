...
class CMR2(object):
    ...
    def leaky_accumulator(self, in_act, noise_vec, x_thresholds):
        ...
        if np.sum(in_act) <= 0:
            ...
            leak_results = (None, self.params['rec_time_limit'], x_s)
        else:
            ...
            leak_results = (winner_index, time, x_s)
    ...
    def recall_session(self):
        ...
        # track & limit number of "blanks" a subj. is allowed to draw during
        # a recall session, to prevent wasting lots of runtime during pso
        # on a param vec that is simply generating None's
        num_of_nones = 0

        # number of items allowed to be recalled in addition to list length
        num_extras = 3

        # run a recall session for the amount of time in rec_time_limit,
        # or until person recalls >= list length + num extras,
