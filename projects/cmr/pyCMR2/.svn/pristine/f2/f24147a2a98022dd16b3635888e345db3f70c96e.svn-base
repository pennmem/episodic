...
class CMR2(object):
    ...
    def present_distractor_before_recall(self):
...
def run_CMR2_singleSubj(data_mat, LSA_mat, params, source_info):
    ...
    #TODO print('Line below this is not in the original code: updated for CMR2')
    this_CMR.present_distractor_before_recall()
...
def main():
    ...
    # save CMR2 results
    np.savetxt('resp_LTP228.txt', np.asmatrix(rec_nos), delimiter=',', fmt='%i')
    np.savetxt('times_LTP228.txt', np.asmatrix(times), delimiter=',', fmt='%i')
