    ...
    for trial in range(len(data_recs)):
        ...
        for rec in this_list:
            if rec == 0:
                ...
                serial_pos = int(np.where(pres_list==int(rec))[0])+1
...
def get_spc_pfc(rec_lists, ll, onlysixpoints=True):
    ...
    rec_lists = np.asarray(split_sessions(np.asarray(rec_lists)))
...
def main():
    ...
    if not os.path.isfile(data_path):
            import create_subj_txts
            create_subj_txts.create_txts_for_subj(fnum)
    ...
    S = 50
    I = 16
    noise_maker.main(S, I)
    xopt, fopt = pso(obj_func, lb, ub, swarmsize=S, maxiter=I, debug=False, phig=.5)
    ...
    np.savetxt('xopt_LTP%d.txt' % fnum, xopt, delimiter=',', fmt='%f')
