...
5) fitting/genalg_cmr2_ltpFR3.py - Contains a Python function for running genetic
	algorithm optimization on CMR2.
6) fitting/orthant_search.py - Contains a Python function for running a test of
	CMR2 parameters in every orthant of the specified search space (a glorified
	grid search). 
7) fitting/optimization_utils.py - Contains a variety of utility functions used by the
	...
	behavior.
8) fitting/noise_maker_pso.py - Contains a function used by the PSO algorithm to generate
	random numbers used in particle initialization and motion.
9) fitting/noise_maker_ga.py - Contains a function used by the genetic algorithm to
	generate random numbers used in intializing, crossing, and mutating parameter
	sets.

Semantic similarity matrices and their associated wordpools are stored as text
files in the wordpools/ subdirectory:
10) wordpools/PEERS_wordpool.txt - Contains a list of the 1638 words in the PEERS
	wordpool. Word ID numbers correspond to the ordering of words in this list,
	from 1 to 1638.
11) wordpools/PEERS_w2v.txt - Contains a matrix of semantic similarities between
...
algorithm you like, included are three algorithms that have been used in
previous work. They can be found in the fitting/ subdirectory. As the names of
the files imply, the provided code was specialized for fitting ltpFR3. In order
...
In all three algorithms, parallel instances will automatically track one
another's progress to make sure the next iteration/generation starts once all
jobs have finished evaluating the current iteration/generation. You therefore
only need to run pgo once, rather than once per iteration.

IMPORTANT: These functions leave behind many files tracking intermediate steps
...
2) genalg_cmr2_ltpFR3.py: A genetic algorithm. In contrast to particle swarms,
genetic algorithms are designed to test large numbers of parameter sets for
relatively small numbers of generations. Parameter sets within a generation can
be tested in parallel. Each new generation cannot start until all parameter
sets from the current generation have finished.

3) orthant_search.py: A form of grid search in which some predefined number of
parameter sets are tested at random within each orthant of the search space.
This is highly inefficient, but can be useful for ensuring that every quadrant
of the search space is investigated. (See some of Karl Healey's papers for a
description of a similar procedure.) As this search does not occur in an
iterative/generational design, any number of parameter sets can be run in
parallel, and jobs never need to wait for others to finish before proceeding.
