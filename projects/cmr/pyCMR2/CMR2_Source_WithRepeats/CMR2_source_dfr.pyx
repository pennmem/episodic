...
class CMR2(object):
    ...
    def __init__(self, params, nsources, source_info, LSA_mat, data_mat):
        ...
        self.all_session_items_unique = np.unique(self.all_session_items_sorted)

        self.nunique_study_items = len(self.all_session_items_unique)
        ...
        self.templength = self.nunique_study_items + self.ndistractors

        # total number of elements operating in the system,
        # including all study lists, distractors, and sources
        self.nelements = (self.nunique_study_items + self.ndistractors +
                          self.nsources)
        ...
        self.exp_LSA = np.zeros(
            (self.nunique_study_items, self.nunique_study_items),
                                dtype=np.float32)

        # Get list-item LSA indices
        for row_idx, this_item in enumerate(self.all_session_items_unique):

            # get item's index in the larger LSA matrix
            this_item_idx = this_item - 1

            for col_idx, compare_item in enumerate(self.all_session_items_unique):
        ...
        self.distractor_idx = self.nunique_study_items
        ...
        self.n_prior_recalls = np.zeros(
            [self.nunique_study_items, 1], dtype=np.float32)
    ...
    def create_semantic_structure(self):
        ...
        self.M_CF[:self.nunique_study_items, :self.nunique_study_items] \
            += cf_exp_LSA
        ...
        self.M_FC[:self.nunique_study_items, :self.nunique_study_items] \
            += fc_exp_LSA
    ...
    def recall_session(self):
        ...
        # CBT manipulation - decrease rumination time during BAT
        if self.list_idx > 79:
            self.params['rec_time_limit'] = 200
        elif self.list_idx > 99:
            self.params['rec_time_limit'] = 75000
        ...
        nitems_in_session = self.nunique_study_items
        ...
        while ((time_passed < rec_time_limit)
            ...
            if winner_accum_idx is not None:
                ...
                winner_ID = np.unique(self.all_session_items)[winner_sorted_idx]
                ...
                # item_source_code = self.all_session_item_codes[winner_sorted_idx]
            ...
            if self.learn_while_retrieving:
                ...
                self.M_CF += np.multiply(M_CF_exp, self.L_CF)
    ...
    def present_item(self, item_idx):
        ...
        if item_idx != self.distractor_idx:
            ...
            source_indices, = np.where(
                self.all_session_items == self.all_session_items_unique[item_idx])
    ...
    def update_context_temp(self):
        ...
        nelements_temp = self.nunique_study_items + 2*self.nlists + 1
    ...
    def present_list(self):
        ...
        thislist_pres_indices = np.searchsorted(
            self.all_session_items_unique, thislist_pattern)
        ...
        for i in range(self.listlength):
            ...
            #________ Track event information
            # get item emotional valence

            # get loc of this item's source information
            source_indices, = np.where(
                self.all_session_items == self.all_session_items_unique[presentation_idx])

            # get item's source info
            source_idx = source_indices[0]
            item_source_code = self.all_session_item_codes[source_idx]
            item_id = self.all_session_items_unique[presentation_idx]
        ...
        # Update context shift rate to the 'beta_distract' parameter
...
def run_CMR2(LSA_path, LSA_mat, data_path, params, sep_files,
    ...
    if not sep_files:
        ...
        for m, pres_sheet in enumerate(subj_presented_data):
            ...
            output_tuple = run_CMR2_singleSubj(
                pres_sheet, LSA_mat,
                params=params, source_info=subj_source_sheets[m],
                nsource_cells=nsource_cells)

            resp_Subj = output_tuple[0]
            RT_Subj = output_tuple[1]
            time_Subj = output_tuple[2]
            context_reshaped = output_tuple[3]
            context_timepoint = output_tuple[4]
            event_timepoint = output_tuple[5]
            emot_timepoint = output_tuple[6]
            encode_timepoint = output_tuple[7]
